...
function renderGlobalChartsFinal(container) {
    ...
    if (currentMode === 'revenus') {
        if (revenueSortAsc) {
            ...
            items = items.filter(i => i.value > 0).sort((a, b) => a.value - b.value);
        } else {
    ...
    prevBtn.onclick = () => {
        if (chartPage > 0) {
            chartPage--;
            // Re-render only chart area logic to avoid full reset
            // But here we can just call renderGlobalChartsFinal(area) 
            // We need 'container' reference. The function receives it.
            // However, clearing container is done in renderStats... 
            // Better strategy: Clean container here or call renderStats? 
            // renderStats resets page. So we must clear and call this function again.
            // OR: modify function to not append but replace? 

            // Simple fix: We know 'container' is 'stats-chart-area'.
            const area = document.getElementById('stats-chart-area');
            // Clear it (except toggle button if exists, which is tricky)
            // Actually renderGlobalChartsFinal assumes it appends. 
            // Let's clear container's chart part.

            // RE-CALL renderStats() resets page. So we can't use it.
            // We have to clear the chart-area manually here or change architecture.
            // Let's just clear the area CAREFULLY.
            // 'container' passed here is usually #stats-chart-area.

            // If we are in 'revenus', there is a toggle button BEFORE this function is called.
            // This function 'renderGlobalChartsFinal' is responsible for the CHART and PAGINATION.
            // So we should clear the chart and pagination, but NOT the toggle button.
            // BUT previously, renderGlobalChartsFinal was appending to 'container'.

            // FIX: We need a dedicated wrapper for chart+pagination to clear easily.
            renderStatsWithPage(chartPage - 1); // Pass explicit page? No, update state then render.
        }
    };
    ...
    // 4b. Clear previous chart content if we are refreshing
    // To support re-rendering within this function, we need to check if we should clear.
    // The easiest way is to modify renderStats to create a 'chart-wrapper-inner' and pass THAT.
    container.appendChild(pagContainer);


    // 5. Create Canvas
    const canvas = document.createElement('canvas');
    // Height is fixed for 15 items to fit in generic screen
    // 15 items * ~30px = 450px. + Padding.
    // Screen height ~800px. Header ~100px.
...
function changePage(delta) {
    ...
    // Re-Render ONLY the chart part
    // We need to clear the container but preserve the revenue toggle if it exists.
    // The 'renderGlobalChartsFinal' function appends.
    // So we need to clear the specific area allocated for charts.

    // Better Approach: renderStats() clears everything.
    // We can define a helper that clears only chart elements?

    // Quick Fix:
    const area = document.getElementById('stats-chart-area');

    // Remove everything EXCEPT the Revenue Toggle Button (which is the first child if in revenue mode)
    // In revenue mode, there is a DIV (toggle) then the chart stuff.
    // In other modes, just chart stuff.

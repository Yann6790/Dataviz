...
const DataStore = {
    ...
    geometries: {}, // INSEE -> Turf Feature (Polygon/MultiPolygon)
    bboxes: {} // INSEE -> [minX, minY, maxX, maxY]
};
...
async function processClayData(url) {
    updateLoading("Analyse des sols (Intersection des Polygones)...");
    document.getElementById('loading-sub').innerText = "Cela peut prendre quelques secondes...";

    // Use PapaParse on the URL
    // New CSV Format: Geo Point, Geo Shape, gid, geom_err, ident, cdate, mdate, alea
    // Delimiter: ","
    Papa.parse(url, {
        download: true,
        header: true,
        delimiter: ",", // New delimiter for risque_argile.csv
        skipEmptyLines: true,
        step: function (row) {
            ...
            try {
                // 1. Extract Risk Geometry
                const geometry = JSON.parse(data['Geo Shape']); // Parsing JSON string
                let riskLevel = (data.alea || "FAIBLE").toUpperCase();

                // Map specific CSV values like "INCONNU" explicitly
                if (riskLevel === "INCONNU") riskLevel = "FAIBLE";

                // 2. Create Turf Feature for Risk Zone
                const riskPoly = turf.feature(geometry);
                const riskBBox = turf.bbox(riskPoly); // Calculate Risk BBox once

                // 3. Check Intersection with ALL Cities (Polygon-Polygon Intersection)
                // Optimized with Bounding Box Check -> 100x Faster
                Object.keys(DataStore.geometries).forEach(insee => {
                    const cityBBox = DataStore.bboxes[insee];

                    // FAST FAIL: Check if BBoxes overlap
                    // [minX, minY, maxX, maxY]
                    if (riskBBox[2] < cityBBox[0] || riskBBox[0] > cityBBox[2] ||
                        riskBBox[3] < cityBBox[1] || riskBBox[1] > cityBBox[3]) {
                        return; // No overlap possible, skip heavy intersection
                    }

                    const cityFeature = DataStore.geometries[insee];

                    if (turf.booleanIntersects(riskPoly, cityFeature)) {
                        // Intersection Found!
